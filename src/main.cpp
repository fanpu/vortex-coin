...
const string strMessageMagic = "VortexCoin Signed Message:\n";
...
{
    int64 nSubsidy = 25 * COIN; //Amt of coins per block


//    if(nHeight < 17280) // no block reward within the first 3 days
//        nSubsidy = 0;
    if(nHeight > 210384000) // no block reward after 100 years
        nSubsidy = 0;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.35 * 24 * 60 * 60; // VortexCoin: 0.35 days (time it readjusts the difficulty of mining)
static const int64 nTargetSpacing = 15; // VortexCoin: 15 seconds (ideal time it takes for a block to be found)
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal VortexCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VortexCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	CBlock(hash=14dc23f54de47df79717, PoW=00000aeb1f05fbf0b2fa, ver=1, 			hashPrevBlock=00000000000000000000, hashMerkleRoot=ef7e256a2c, 			nTime=1399574511, nBits=1e0ffff0, nNonce=2086299649, vtx=1)
  	...
  	vMerkleTree: ef7e256a2c */
        ...
        // Genesis block
        const char* pszTimestamp = "And thus VortexCoin was born";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
